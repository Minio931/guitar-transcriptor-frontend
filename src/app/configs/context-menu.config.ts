...
export function getContextMenuItems(contextMenuService: ContextMenuService): MenuItem[] {
  return [
    ...
    },
    {
      ...
      command: () => contextMenuService.tabContextAction(TabObjectType.Pause, NoteEnum.WHOLE_NOTE),
    },
    {
      label: '1/2',
      icon: "half-pause-svg-icon",
      command: () => contextMenuService.tabContextAction(TabObjectType.Pause, NoteEnum.HALF_NOTE),
    },
    {
      label: '1/4',
      icon: "quarter-pause-svg-icon",
      command: () => contextMenuService.tabContextAction(TabObjectType.Pause, NoteEnum.QUARTER_NOTE),
    },
    {
      label: '1/8',
      icon: "eighth-pause-svg-icon",
      command: () => contextMenuService.tabContextAction(TabObjectType.Pause, NoteEnum.EIGHTH_NOTE),
    },
    {
      label: '1/16',
      icon: "sixteenth-pause-svg-icon",
      command: () => contextMenuService.tabContextAction(TabObjectType.Pause, NoteEnum.SIXTEENTH_NOTE),
    },
    {
      label: '1/32',
      icon: "thirty-second-pause-svg-icon",
      command: () => contextMenuService.tabContextAction(TabObjectType.Pause, NoteEnum.THIRTY_SECOND_NOTE),
    },
    // {
    //   label: 'Arpeggio',
